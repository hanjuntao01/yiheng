...
$huoyuan = trim($_REQUEST['huoyuan']);
$country = trim($_REQUEST['m_country']);// ecmoban hky
if ($user_id <= 0) {
...
if ($sf_agreement != 1) {
    ...
    if(!empty($huoyuan))
    {
        $sql = "UPDATE ".$GLOBALS['ecs']->table('merchants_steps_fields')." SET `source`= '{$huoyuan}',country='{$country}' WHERE user_id = '$user_id'";// ecmoban hky
