...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    else
    {
        ...
        // 更新：添加到购物车
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent, $warehouse_id, $area_id, $goods->stages_qishu,$goods->store_id,$goods->end_time,$goods->store_mobile)) { //ecmoban模板堂 --zhuo $goods->warehouse_id
            if (!empty($goods->divId)) {
                $result['divId'] = $goods->divId;
            }
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if(isset($_COOKIE['rel_name']) && isset($_COOKIE['id_num']))
    {
        $smarty->assign('rel_name',$_COOKIE['rel_name']);
        $smarty->assign('id_num',$_COOKIE['id_num']);
    }
	...
	$sql="SELECT value FROM ".$GLOBALS['ecs']->table('shop_config')." WHERE code = 'limited_amount'";
	$limited_amount = doubleval($GLOBALS['db']->getOne($sql));
	$smarty->assign('limited_amount' , $limited_amount);
	...
	foreach($cart_goods_list as $key => $val)
	{
		if($val['ru_id'] != 0)
		{
			$sql="SELECT source FROM ".$GLOBALS['ecs']->table('merchants_steps_fields')." WHERE user_id='{$val['ru_id']}'";
			$ru_huoyuan=$GLOBALS['db']->getOne($sql);
			
			if($ru_huoyuan != "国内仓库")
			{
				$is_kj = 1;
				
				foreach($val['goods_list'] as  $value)
				{
					$arr[$key]['ru_id'] = $val['ru_id']; 
					$arr[$key]['price'] += $value['subtotal'];
					$arr[$key]['number'] += $value['goods_number'];
				}
				foreach($arr as $key1 => $val1)
				{
					if($val1['number'] > 1 && $val1['price'] > $limited_amount)
					{
						show_message('有商店商品超过限定金额,无法购买！');
					}
					$i++;
				}
			}
    ...
    foreach($cart_goods_list as $key => $val)
    {
        if($val['ru_id'] != 0)
        {
            ...
            if($res!='国内仓库')
            {
                $sql="SELECT value FROM ".$GLOBALS['ecs']->table('shop_config')." WHERE code='duty_free'";
                $res=$GLOBALS['db']->getOne($sql);
                
                foreach($val['goods_list'] as $key1 => $val1)
                {
                    if($val1['free_rate'] == 0)
                    {
                        $sql="SELECT c.rate FROM ".$GLOBALS['ecs']->table('goods')." AS g ".
                        "LEFT JOIN ".$GLOBALS['ecs']->table('category')." AS c ON g.cat_id=c.cat_id ".
                        " WHERE g.goods_id = {$val1['goods_id']}";
                        $rate=$GLOBALS['db']->getOne($sql);
                        $rate_price = round($val1['goods_price']*$val1['goods_number'] *  ($rate / 100),2);
                        
                        $rate_arr[$i]['goods_id'] = $val1['goods_id'];
                        $rate_arr[$i]['rate_price'] = $rate_price;
                        $i++;
                    }
                }
            }
    ...
    $smarty->assign('rate_arr',$rate_arr);
    
    $rate_price = 0;
    foreach($rate_arr as $key => $val)
    {
        $rate_price=floatval($rate_price)+floatval($val['rate_price']);
    }
    
    $smarty->assign('rate_price',$rate_price);
$total['amount'] = round(floatval($total['amount'])+floatval($rate_price),2);
    $total['amount_formated'] = "<em>¥</em>".$total['amount'];
        ...
        && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS))
    {
        ...
        if (defined('THEME_EXTENSION')) {
            $bonus = get_user_bouns_new_list($_SESSION['user_id'], 1, 0, 'bouns_available_gotoPage', 0, 20); //获取用户全部的红包列表，显示20
            //获取不能使用的优惠券数组
    ...
    if ($_CFG['use_coupons'] == 1 && $flow_type == CART_GENERAL_GOODS)
    {
        ...
        if (defined('THEME_EXTENSION')) {
            $coupons_list = get_user_coupons_list($_SESSION['user_id'], true); //获得当前登陆用户所有的优惠券
            //获取不能使用的优惠券数组
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    if ($_REQUEST['act'] == 'balance') {
        ...
        $order = array(
            ...
            'rel_name'       =>trim($_POST['name']),
            'id_num'         =>trim($_POST['id_num']),
            'invoice_type' => isset($_POST['invoice_type']) ? intval($_POST['invoice_type']) : 0,
        );
        setcookie('rel_name', $_POST['name'], gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'], $GLOBALS['cookie_domain']);   
        setcookie('id_num', $_POST['id_num'], gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'], $GLOBALS['cookie_domain']);
        /* 扩展信息 */
        ...
        if ($is_cart && $is_done == 0) {
            ...
            if (file_exists(MOBILE_DRP)) {
                ...
                $db->query($sql);
                foreach($rate_arr as $key => $val)//插入跨境税费
                {
                    $sql="UPDATE ".$GLOBALS['ecs']->table('order_goods')." SET rate_price = '{$val['rate_price']}',goods_price = '{$val['goods_price']}' WHERE order_id = '{$new_order_id}' AND goods_id = '{$val['goods_id']}'";
                    $GLOBALS['db']->query($sql);
                }
                //微分销end
            } else {
...
/*------------------------------------------------------*/
 ...
 elseif ($_REQUEST['step'] == 'ajax_update_cart') {
    ...
    if (isset($_POST['rec_id']) && isset($_POST['goods_number'])) {
        ...
        foreach($cart_goods['goods_list'] as $key1=>$val)// mobantang -- hky
        {
            if($val['ru_id'] != 0 && $val['free_rate'] == 0)
            {
                $sql="SELECT source FROM ".$GLOBALS['ecs']->table('merchants_steps_fields')." WHERE user_id={$val['ru_id']}";
                $res1=$GLOBALS['db']->getOne($sql);
                
                if($res1!='国内仓库')
                {               
                    $sql="SELECT c.rate FROM ".$GLOBALS['ecs']->table('goods')." AS g ".
                    "LEFT JOIN ".$GLOBALS['ecs']->table('category')." AS c ON g.cat_id=c.cat_id ".
                    " WHERE g.goods_id = {$val['goods_id']}";
                    $rate=$GLOBALS['db']->getOne($sql);
                    
                    $cart_goods['goods_list'][$key1]['rate_price'] = round($val['amount'] *  ($rate / 100),2);
                }
            }
            else
            {
                $cart_goods['goods_list'][$key1]['rate_price'] = 0;
            }
            $cart_total_rate+=$cart_goods['goods_list'][$key1]['rate_price'];
        }
        ...
        foreach ($cart_goods['goods_list'] as $goods)
        {
            if ($goods['rec_id'] == $key)
            {
                ...
                {
                    $result['goods_subtotal'] = $result['goods_subtotal']."<div style='font-size:12px;color:#666;'>税费:{$goods['rate_price']}</div>";
                }
        ...
        if($subtotal_number > 1 && $cart_goods['total']['goods_amount'] > $limited_amount)
        {
            $result['can_buy'] = -1;
        }
        else
        ...
        $cart_goods['total']['goods_price']=str_replace('<em>¥</em>','',$cart_goods['total']['goods_price']);
        $cart_goods['total']['goods_price']=$cart_goods['total']['goods_price']+$cart_total_rate;
        $cart_goods['total']['goods_price']='<em>¥</em>'.$cart_goods['total']['goods_price'];
       ...
       /* 返回购物车信息 */
        ...
        if ($act_id > 0) {
            ...
            $result['act_id'] = $act_id;
                ...
                //购物车判断商品是否超出限制金额
        $cart_goods = get_cart_goods($cValue, 1, @$_COOKIE['flow_region'], $area_id);
        $sql="SELECT value FROM ".$GLOBALS['ecs']->table('shop_config')." WHERE code = 'limited_amount'";
        $limited_amount = doubleval($GLOBALS['db']->getOne($sql));
        $arr=array();
        $result['can_buy']=array();
        foreach($cart_goods['goods_list'] as $key => $val)
        {
            if($val['ru_id'] != 0)
            {
                ...
                if($ru_huoyuan != "国内仓库")
                {
                    ...
                    {
                        @$arr[$key]['ru_id'] = $val['ru_id']; 
                        @$arr[$key]['price'] += $value['amount'];
                        @$arr[$key]['number'] += $value['goods_number'];
                    }
                    foreach($arr as $key1 => $val1)
                    {
                        $result['can_buy'][$key1]=array();
                        if($val1['number'] > 1 && $val1['price'] > $limited_amount)
                        {
                            $result['can_buy'][$key1]['ru_id'] = $val1['ru_id'];
                            $result['can_buy'][$key1]['err'] = 1;
                        }
                        else
                        {
                            $result['can_buy'][$key1]['ru_id'] = $val1['ru_id'];
                            $result['can_buy'][$key1]['err'] = 0;
                        }
                    }
                }
            }           
...
elseif ($_REQUEST['step']== 'ajax_cart_goods_amount')
{
    ...
    $goods_amount['subtotal_amount']=str_replace('<em>¥</em>','',$goods_amount['subtotal_amount']);

        $cart_total_rate=0;
    
    foreach($cart_goods as $key1=>$val)// mobantang -- hky
    {
        if($val['ru_id'] != 0 && $val['free_rate'] == 0)
        {
            $sql="SELECT source FROM ".$GLOBALS['ecs']->table('merchants_steps_fields')." WHERE user_id={$val['ru_id']}";
            ...
            {   
                ...
                $cart_goods[$key1]['rate_price'] += round($val['subtotal'] *  ($rate / 100),2);
            }
        }
        else
        {
            $cart_goods[$key1]['rate_price'] = 0;
        }
        $cart_total_rate+=$cart_goods[$key1]['rate_price'];
    }
    $result['cart_total_rate'] = $cart_total_rate;
        ...
        foreach($cart_goods as $key => $val)
        {
            if($val['ru_id'] != 0)
            {
                ...
                if($ru_huoyuan != "国内仓库")
                {
                    @$arr[$val['ru_id']]['ru_id'] = $val['ru_id']; 
                    @$arr[$val['ru_id']]['price'] += $val['subtotal'];
                    @$arr[$val['ru_id']]['number'] += $val['goods_number'];
                    foreach($arr as $key1 => $val1)
                    {
                        $result['can_buy'][$key1]=array();
                        if($val1['number'] > 1 && $val1['price'] > $limited_amount)
...
else
{
    ...
    foreach($cart_goods['goods_list'] as $key1=>$val)// mobantang -- hky
    {
        if($val['ru_id'] != 0 && $val['free_rate'] == 0)
        {
            ...
            if($res1!='国内仓库')
            {   
                foreach($val['goods_list'] as $key2=>$val2)
                {       
                $sql="SELECT c.rate FROM ".$GLOBALS['ecs']->table('goods')." AS g ".
                "LEFT JOIN ".$GLOBALS['ecs']->table('category')." AS c ON g.cat_id=c.cat_id ".
                " WHERE g.goods_id = {$val2['goods_id']}";
                $rate=$GLOBALS['db']->getOne($sql);
                
                $cart_goods['goods_list'][$key1]['rate_price'] += round($val2['amount'] *  ($rate / 100),2);
                }
            }
        }
        else
    ...
    $smarty->assign('cart_total_rate',$cart_total_rate);
    $i=0;
    foreach($cart_goods['goods_list'] as $key => $val)
    {
        if($val['ru_id'] != 0)
            ...
            if($res!='国内仓库')
            {
                ...
                foreach($val['goods_list'] as $key1 => $val1)
                {
                    if($val1['free_rate'] == 0)
                    {
                        ...
                        " WHERE g.goods_id = {$val1['goods_id']}";
                        $rate=$GLOBALS['db']->getOne($sql);
                        $rate_price = round($val1['goods_amount'] *  ($rate / 100),2);
                        
                        $cart_goods['total']['goods_amount']=round(doubleval($cart_goods['total']['goods_amount'])+doubleval($rate_price),2);
                        $cart_goods['total']['goods_price']="<em>¥</em>".$cart_goods['total']['goods_amount'];
                        
                        $rate_arr[$i]['rec_id'] = $val1['rec_id'];
                        $rate_arr[$i]['rate_price'] = $rate_price;
                        $i++;
                    }
                }
        ...
        if($ru_huoyuan != '国内仓库')
            {
                ...
                {
                    $arr[$key]['ru_id'] = $val['ru_id']; 
                    $arr[$key]['price'] += $value['goods_amount'];
                    $arr[$key]['number'] += $value['goods_number'];
                }
                foreach($arr as $key1 => $val1)
                {
    ...
    $can_buy = array_unique($can_buy);
    
    
    $smarty->assign('can_buy' , $can_buy);  
...
function flow_cart_stock($arr, $store_id = 0, $warehouse_id = 0, $area_id = 0)
{
    ...
    foreach ($arr AS $key => $val)
    {
        ...
        $sql = "SELECT `goods_id`, `goods_attr_id`, `extension_code`, `warehouse_id` ,`ru_id` FROM" .$GLOBALS['ecs']->table('cart').
               " WHERE rec_id='$key' AND ". $sess_id;
        $goods = $GLOBALS['db']->getRow($sql);
