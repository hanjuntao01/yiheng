...
else
 {
    ...
    if ($search_type == 0) { //搜索商品
        ...
        if ($cat['grade'] > 1) {
            ...
            if ($brand) {
                $price_sel_msb = "(g.brand_id IN(SELECT msb.bid FROM " . $GLOBALS['ecs']->table('brand') . " AS b, " . $GLOBALS['ecs']->table('link_brand') . " AS lb, " . $GLOBALS['ecs']->table('merchants_shop_brand') . " AS msb" .
            ...
            $sql = "SELECT min(IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price))) AS min, " .
                    ...
                    $leftJoin .
                    " WHERE ($children OR " . get_extension_goods($children) . ') AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 ' . $keywords . $tag_where . $price_grade_where;
            //获得当前分类下商品价格的最大值、最小值
        ...
        $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num " . $brand_select .
                ...
                " LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON g.brand_id = b.brand_id AND g.user_id = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 $brand_tag_where $keywords " .
        ...
        $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num, g.user_id " . $brand_select .
                ...
                " LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON g.brand_id = msb.bid AND g.user_id > 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 $brand_tag_where $keywords " .
                " LEFT JOIN " . $GLOBALS['ecs']->table('goods_cat') . " AS gc ON g.goods_id = gc.goods_id " .
                " WHERE $children OR " . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id), $cat_keys))) . " AND b.is_show = 1 " .
                "GROUP BY b.brand_id HAVING goods_num > 0 $where_having ORDER BY b.sort_order, b.brand_id ASC";
        ...
        if ($brand) {
            ...
            $sel_msb = "(g.brand_id IN(SELECT msb.bid FROM " . $GLOBALS['ecs']->table('brand') . " AS b, " . $GLOBALS['ecs']->table('link_brand') . " AS lb, " . $GLOBALS['ecs']->table('merchants_shop_brand') . " AS msb" .
                    " WHERE b.is_show = 1 AND b.brand_id = lb.brand_id AND lb.bid = msb.bid AND msb.is_show = 1 AND msb.audit_status = 1 $sel_where_brand) AND g.user_id > 0)";
        ...
        $sql = "SELECT COUNT(*) FROM " . $ecs->table('goods') . " AS g " .
                ...
                "AND (( 1 " . $categories . $keywords . $where_brand . $where_price_min . $where_price_max . $intro . $outstock . " ) " . $tag_where . " ) ".$act_name.
        ...
        /* 查询商品 */        
        $sql = "SELECT pa.*, g.goods_id, g.is_shipping, g.user_id, g.goods_name, g.shop_price,g.market_price, g.is_new, g.comments_number, g.sales_volume, g.is_best, g.is_hot,g.store_new, g.store_best, g.store_hot, " .
                $shop_price .
                ...
                "AND (( 1 " . $categories . $keywords . $where_brand . $where_price_min . $where_price_max . $intro . $outstock . " ) " . $tag_where . " ) ) " .$act_name . $where_price_min . $where_price_max .
                " GROUP BY g.goods_id ORDER BY g.$sort $order"; //增加表别名 g. by wanganlin
        ...
        while ($row = $db->FetchRow($res)) {
            ...
            if ($row['act_id']) {
                ...
                $arr[$idx]['pshop_url'] = build_uri('merchants_index', array('merchant_id' => $row['ru_id']), $row['shop_name']); //by yx
            ...
            $arr[$idx]['rz_shopName'] = get_shop_name($row['user_id'], 1); //店铺名称	
