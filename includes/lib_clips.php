...
function get_collection_goods($user_id, $record_count, $page, $pageFunc, $size = 10)
{
    ...
    $sql = 'SELECT g.goods_thumb, g.user_id, g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, '.
...
function get_collection_brands($user_id, $record_count, $page, $pageFunc, $size = 5)
{
    ...
    $sql = "(SELECT cb.*, b.brand_name, b.brand_logo FROM " . $GLOBALS['ecs']->table('collect_brand') .
            " AS cb LEFT JOIN " . $GLOBALS['ecs']->table('brand') . " AS b ON cb.brand_id = b.brand_id WHERE cb.ru_id = 0 AND b.brand_id > 0 AND cb.user_id = '$user_id')
            UNION
            (SELECT cb_new.*, msb.brandName AS brand_name, msb.brandLogo AS brand_logo FROM " . $GLOBALS['ecs']->table('collect_brand') .
            " AS cb_new LEFT JOIN " . $GLOBALS['ecs']->table('merchants_shop_brand') . " AS msb ON cb_new.brand_id = msb.bid "
            . "WHERE cb_new.ru_id > 0 AND msb.bid > 0) ORDER BY rec_id DESC " .$limit;
    $res = $GLOBALS['db'] -> getAll($sql);
    ...
    foreach($res as $key=>$row)
    {
        ...
        $mbid = ($row['brand_id']>0 && $row['ru_id']) ? $row['brand_id'] : 0;
...
function brand_get_goods($brand_id, $mbid = 0, $cate, $size, $page, $sort, $order, $warehouse_id = 0, $area_id = 0, $act = '',$ship='',$price_min = 0,$price_max = 0, $self = 0)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.user_id, g.goods_name, g.market_price, g.shop_price AS org_price,g.sales_volume, ' .
				$shop_price .
                "IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]'), g.shop_price * '$_SESSION[discount]')  AS shop_price, " .
				"IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price, " .
            ...
            "WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND $brand_where $cate_where $tag_where ".
            "ORDER BY $sort $order";
