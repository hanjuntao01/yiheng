...
// *拍卖活动列表
// */
// function get_user_auction($user_id, $record_count, $page, $pagesize = 10){
	// require_once('includes/cls_pager.php');
	// if($order){
        // $idTxt = $order->idTxt;
        // $keyword = $order->keyword;
    ...
    // /* 取得拍卖订单列表 */
	// $sql = "SELECT og.ru_id, oi.main_order_id, oi.consignee,oi.pay_name, oi.order_id, oi.order_sn, oi.order_status, oi.shipping_status, oi.pay_status, oi.add_time, oi.shipping_time, oi.auto_delivery_time, oi.sign_time, " .
           // "(oi.goods_amount + oi.shipping_fee + oi.insure_fee + oi.pay_fee + oi.pack_fee + oi.card_fee + oi.tax - oi.discount) AS total_fee, og.goods_id, ".
           ...
           // " WHERE oi.user_id = '$user_id' and oi.is_delete = '$is_delete' " .$where.
            // " and (select count(*) from " .$GLOBALS['ecs']->table('order_info'). " as oi2 where oi2.main_order_id = oi.order_id) = 0 " .  //主订单下有子订单时，则主订单不显示
			// " AND oi.is_zc_order = 0 AND extension_code='auction'" . //排除众筹订单 by wu
            // " group by oi.order_id ORDER BY oi.add_time DESC " . $limit;
    
    // $order_list = array('item'=> $arr, 'pager' => $pager, 'record_count' => $record_count);
    // return $order_list;   
// }
?>