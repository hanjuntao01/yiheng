    ...
    if ($type == 0) {
        foreach ($newInfo as $key => $row) {
            ...
            for ($j = 0; $j < count($row); $j++) {
               ...
               $goods_amount = $row[$j]['goods_price'] * $row[$j]['goods_number'] + $row[$j]['rate_price'];
                if ($goods_amount > 0) {
...
function get_cart_check_goods($cart_goods, $rec_id = '', $type = 0){
   $arr['subtotal_amount'] = 0;
    ...
    if(!empty($rec_id)){
        if($cart_goods){
            foreach($cart_goods as $row)
            {
                ...
                if($row['ru_id'] != 0 && $row['free_rate'] == 0)
                {
                    $sql="SELECT source FROM ".$GLOBALS['ecs']->table('merchants_steps_fields')." WHERE user_id={$row['ru_id']}";
                    $res=$GLOBALS['db']->getOne($sql);
                    ...
                    if($res!='国内仓库')
                    {
                        ...
                        $rate_price = round($row['subtotal'] *  ($rate / 100),2);
                        $arr['subtotal_rate'] += $rate_price;
                        
                        $arr['subtotal_amount']=round(doubleval($arr['subtotal_amount'])+doubleval($rate_price),2);
    ...
    $arr['subtotal_rate'] = price_format($arr['subtotal_rate'], false);
    return $arr;
}
...
function goodsShippingFee($goods_id = 0, $warehouse_id = 0, $region = array(), $seckill_price = '') {
    ...
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            $GLOBALS['ecs']->table('area_region') . ' AS r ' .
            'WHERE ' . $where .
            " AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.enabled = 1 AND a.ru_id = '" . $goodsInfo['user_id'] . "' GROUP BY s.shipping_id ORDER BY s.shipping_order"; //by wu
...
function get_goods_transport_tpl($goodsInfo = array(), $region = array(), $shippingInfo = array(), $goods_number = 1){
    ...
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            $GLOBALS['ecs']->table('area_region') . ' AS r ' .
            'WHERE r.region_id ' . db_create_in($region) .
            " AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id " . $where .
            " AND s.enabled = 1 AND a.ru_id = '" . $goodsInfo['user_id'] . "'";
    $val = $GLOBALS['db']->getRow($sql);
...
function get_order_transport($goods_list, $consignee, $shipping_id, $shipping_code){
    ...
    if($goods_list && $shipping_code != 'cac'){
        ...
        $arr = array(
            'sprice' => $sprice + $unified_total, //固定运费 + 运费模板
...
function get_user_coupons_list($user_id = '', $is_use = false, $total = '', $cart_goods = false, $user = true) {
    ...
    if ($is_use && isset($total) && $cart_goods) {
        ...
        foreach($res as $key=>$row){
            ...
            $sql = "SELECT c.*, cu.uc_id, s.shop_name FROM " . $GLOBALS['ecs']->table("coupons_user") . " AS cu " .
                    " LEFT JOIN " . $GLOBALS['ecs']->table("coupons") . " AS c ON cu.cou_id = c.cou_id " .
					" LEFT JOIN " . $GLOBALS['ecs']->table("seller_shopinfo") . " AS s ON c.ru_id = s.ru_id " .
                    " WHERE c.review_status = 3 AND c.cou_end_time > $time AND $time > c.cou_start_time" .
                    " AND " . $row['order_total'] . " >= c.cou_man AND c.ru_id = '" .$row['seller_id']. "'" .
                    " AND cu.order_id = 0 AND cu.user_id = '$user_id' AND s.status = 1";
        ...
        $sql = " SELECT c.*,cu.*,o.order_sn,o.add_time, s.shop_name FROM " .
                ...
                " o ON cu.order_id = o.order_id " . " LEFT JOIN " . $GLOBALS['ecs']->table("seller_shopinfo") . " AS s ON c.ru_id = s.ru_id " . " $where ";
...
function shippingArea($ru_id = 0) {
    $sql = " SELECT region_id FROM " . $GLOBALS['ecs']->table('shipping_area') .
            " AS s LEFT JOIN " . $GLOBALS['ecs']->table('area_region') . " AS a ON a.shipping_area_id = s.shipping_area_id WHERE s.ru_id = '$ru_id' ";
