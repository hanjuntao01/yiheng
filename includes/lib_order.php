...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
        ...
        $sql="SELECT SUM(rate_price) FROM ".$GLOBALS['ecs']->table('order_goods')." WHERE order_id = '{$order['order_id']}'";
        $order['rate_price']   = $GLOBALS['db']->getOne($sql); 
        
        if(empty($order['rate_price']))
        {
            $order['rate_price'] = 0;
        }
...
function order_goods($order_id)
{
    $sql = "SELECT og.rec_id, og.goods_id, og.ru_id, og.goods_name, og.goods_sn, og.market_price, og.goods_number, og.warehouse_id, " .
            ...
            "og.goods_price * og.goods_number AS subtotal, og.extension_code,g.shop_price, g.is_shipping, g.goods_weight AS goodsWeight , og.rate_price , g.goods_img, g.goods_thumb, " .
            "g.goods_cause FROM " . $GLOBALS['ecs']->table('order_goods') . " AS og ".
...
function cart_goods($type = CART_GENERAL_GOODS, $cart_value = '', $ru_type = 0, $warehouse_id = 0, $area_id = 0, $consignee = '',$store_id = 0)
{
    ...
    //查询非超值礼包商品
    $sql = "SELECT c.warehouse_id, c.area_id, c.rec_id, c.user_id, c.goods_id, c.ru_id, g.cat_id, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number, g.default_shipping, g.goods_weight as goodsWeight,g.free_rate, " .//储值卡指定分类 liu
	...
	/* 判断购物车商品价格是否与目前售价一致，如果不同则返回购物车价格失效 */
        ...
        $arr[$key]['free_rate']     = $value['free_rate'];
        ...
        if ($value['extension_code'] == 'package_buy')
        {
            ...
            if($value['extension_code'] == 'presale' || $value['rec_type'] > 1 ){
                ...
                if($ru_type == 1 && $warehouse_id > 0 && $store_id == 0){
                    ...
                    $sql = "SELECT IF(g.model_price < 1, g.goods_number, IF(g.model_price < 2, wg.region_number, wag.region_number)) AS goods_number, g.user_id, g.model_attr, g.free_rate FROM " .
                            $GLOBALS['ecs']->table('goods') ." AS g " . $leftJoin .
                            " WHERE g.goods_id = '" .$value['goods_id']. "' LIMIT 1";
                    $goodsInfo = $GLOBALS['db']->getRow($sql);
                    $row['free_rate'] = $goodsInfo['free_rate'];
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = null, $shipping_fee = 0)
{
    ...
    if ($refund_type == 1 || $refund_type == 2) {
        ...
        $sql = "UPDATE " . $GLOBALS['ecs']->table('seller_bill_order') . " SET return_amount = return_amount + '$return_amount', " .
                ...
                "return_shippingfee = return_shippingfee + '$return_shippingfee' ".
    ...
    foreach($shipping_list as $key=>$val)
    {
        if (substr($val['shipping_code'], 0, 5) != 'ship_') {
            if($GLOBALS['_CFG']['freight_model'] == 0){
                ...
                $shipping_fee = ($shipping_count == 0 AND $cart_weight_price['free_shipping'] == 1) ? 0 : shipping_fee($val['shipping_code'], $configure,
                ...
                if ((in_array(trim($brand_id), $get_act_range_ext) && $row1['is_gift'] == 0) || in_array($row1['is_gift'], $favourable_id_list)) {
                    foreach ($sort_favourable['by_brand'] as $key2 => $row2) {
                        ...
                        if ($row1) {
                            ...
                            if ($row1['is_gift'] == $row2['act_id'] && $row1) { // 赠品
                                $merchant_goods[$key]['new_list'][$row2['act_id']]['act_cart_gift'][$row1['rec_id']] = $row1;
                            }
                        }
                    }
...
function get_act_range_ext($user_rank, $user_id = 0,$act_range)
{
    ...
    $user_rank = ',' . $user_rank . ',';
    $now = gmtime();
    $sql = "SELECT act_range_ext, userFav_type, act_range " .
            "FROM " . $GLOBALS['ecs']->table('favourable_activity') .
            " WHERE CONCAT(',', user_rank, ',') LIKE '%" . $user_rank . "%'" .
