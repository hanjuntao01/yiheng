...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        $sql   = 'SELECT goods_id, goods_name, goods_thumb, shop_price FROM ' . $GLOBALS['ecs']->table('goods') .
                " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 order by INSTR('".$_COOKIE['ECS']['history']."',goods_id) limit 0,5";
 ...
 * @return  string
 */
function insert_flow_info($goods_price,$market_price,$saving,$save_rate,$goods_amount,$real_goods_count,$cart_total_rate)
{
    $subtotal_rate = price_format($cart_total_rate);
    $goods_price=str_replace('<em>¥</em>','',$goods_price);
    $goods_price+=$cart_total_rate;
    $goods_price = price_format($goods_price);
    $GLOBALS['smarty']->assign('cart_total_rate',$cart_total_rate);//mobantang -- hky
    $GLOBALS['smarty']->assign('subtotal_rate',$subtotal_rate);//mobantang -- hky
