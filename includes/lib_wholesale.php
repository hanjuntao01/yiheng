...
function get_wholesale_goods_info($act_id, $warehouse_id = 0, $area_id = 0, $select = array()) {
    ...
    if ($row !== false) {
        ...
        $arr['moq'] = $row['moq'];
        //$arr['price_ladder']	= get_price_ladder($row['goods_id']);
        ...
        $arr['goods_extend'] = get_wholesale_extend($row['goods_id']); //获取批发商品标识
        ...
        $arr['goods_weight'] = (intval($row['goods_weight']) > 0) ?
                $row['goods_weight'] . $GLOBALS['_LANG']['kilogram'] :
                ($row['goods_weight'] * 1000) . $GLOBALS['_LANG']['gram'];

        $arr['rz_shopName'] = get_shop_name($row['user_id'], 1); //店铺名称	
        ...
        if ($GLOBALS['_CFG']['open_oss'] == 1) {
            ...
            $arr['shopinfo']['brand_thumb'] = $bucket_info['endpoint'] . $row['shopinfo']['brand_thumb'];
        }
...
function see_more_goods($user_id, $act_id) {
    ...
    if ($res) {
        ...
        foreach ($res as $k => $v) {
            ...
            //$arr[$k]['price'] = get_min_price($v['prices']);
...
function get_wholesale_orders($user_id, $record_count, $page, $where = '', $order = '', $pagesize = 10) {
    ...
    /* 取得订单列表 */
    $arr = array();
    $sql = "SELECT og.ru_id,og.goods_attr ,og.goods_number, oi.inv_type, oi.inv_payee, oi.tax_id, oi.postscript, oi.main_order_id, oi.consignee, oi.order_id,oi.mobile, oi.order_sn, oi.order_status, oi.add_time, " .
...
function wholesale_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true) {
    ...
    if ($res === NULL) {
        ...
        if ($data === false) {
            ...
            // $sql = "SELECT pc.cat_id, COUNT(*) AS goods_num FROM " . $GLOBALS['ecs']->table('wholesale_cat') . " AS pc " .
...
function get_wholesale_child_cat($cat_id = 0, $type = 0) {    
   if ($cat_id > 0) {
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0) {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show, style_icon ' .
