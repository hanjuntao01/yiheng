...
class ucenter extends integrate
{
     ...
     * @return void
     */
    function set_cookie($username='',$email_phone = 0, $is_oath = 1)
    {
        if (empty($username))
        {
            /* 摧毁cookie */
        ...
        else
        {
            if ($is_oath == 1) {
                ...
                $sql = "SELECT u.user_id, u.user_name, u.password, u.email, ua.identifier FROM " . $GLOBALS['ecs']->table('users') . " AS u," .
                        ...
                        " WHERE u.user_id = ua.user_id AND ua.user_name = '$username' LIMIT 1";
                $row = $GLOBALS['db']->getRow($sql);
            }
     ...
     * @return void
     */
    function set_session ($username='', $is_oath = 1)
    {
        if (empty($username))
        {
