...
function get_user_info($id=0)
{
    ...
    if($user['user_name'] != $_SESSION['user_name']){
        ...
        if (isset($_SESSION['identifier']) && !empty($_SESSION['identifier'])) {
            $sql = "SELECT u.user_id, ua.user_name FROM " . $GLOBALS['ecs']->table('users') . " AS u," .
                    $GLOBALS['ecs']->table('users_auth') . " AS ua" .
                    " WHERE u.user_id = ua.user_id AND ua.user_id = '$id' AND ua.identifier = '" . $_SESSION['identifier'] . "' LIMIT 1";
