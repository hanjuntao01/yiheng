<?php
namespace App\Custom\Flow\Controllers;

use App\Http\Flow\Controllers\Index as Foundation;
...
class Index extends Foundation {
    ...
    public function actionIndex() {
        ...
        $this->assign('limited_amount', $limited_amount);
		$i = 0;
        $is_kj = 0;
		foreach($cart_goods_list as $key => $val)
		{
			$sql="SELECT free_rate FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id = '{$val['goods_id']}'";
			$free_rate=$GLOBALS['db']->getOne($sql);
			if($val['ru_id'] != 0)
			{
		...
		foreach($cart_goods_list as $key => $val)
		{
			...
			if($val['ru_id'] != 0)
			{
				...
				if($res!='国内仓库')
				{
					...
					{
						$sql="SELECT free_rate FROM ".$GLOBALS['ecs']->table('goods')." WHERE goods_id = '{$val1['goods_id']}'";
						$free_rate=$GLOBALS['db']->getOne($sql);
						if($free_rate != 1)
						{
							$sql="SELECT c.rate,g.shop_price FROM ".$GLOBALS['ecs']->table('goods')." AS g ".
							"LEFT JOIN ".$GLOBALS['ecs']->table('category')." AS c ON g.cat_id=c.cat_id ".
							" WHERE g.goods_id = {$val1['goods_id']}";
							$rate=$GLOBALS['db']->getAll($sql);
							$rate_price = $val1['subtotal'] *  ($rate[0]['rate'] / 100);
							
							$rate_arr[$i]['goods_id'] = $val1['goods_id'];
							$rate_arr[$i]['rate_price'] = $rate_price;
							$rate_arr[$i]['goods_price'] = floatval($val1['goods_price'])+floatval($rate_price);
							$i++;
						}
					}
				}
		...
		if($rate_price > 0)
		{
			$this->assign('rate_price',price_format($rate_price,false));
		}
		else
		{
			$this->assign('rate_price','');
		}
		//
        ...
        if ($cart_goods_list) {
            foreach ($cart_goods_list as $key => $val) {
                ...
                foreach ($val['goods_list'] as $v) {
                    ...
                    if($v['store_id'] == 0)$isStoreOrder=0;//判断是否门店
    ...
    public function actionDone() {
		...
		$total['amount'] += floatval($rate_price);
		$total['amount_formated'] = '<em>¥</em>'.$total['amount'];
        ...
        if ($order['extension_code'] == 'exchange_goods') {
            ...
            $order['integral_money'] = $order['integral'] / $integral_scale;
        }
        ...
        //
        $order['rel_name'] = $_POST['name'];
        $order['id_num'] = $_POST['id_num'];
        /* 插入订单表 */
        $error_no = 0;
        do {
        ...
        if (is_dir(APP_DRP_PATH)) {
            ...
            if (isset($drp_affiliate['on']) && $drp_affiliate['on'] == 1) {
                $sql = "SELECT u.parent_id FROM {pre}users as u" .
                        " LEFT JOIN  {pre}drp_shop as ds ON u.parent_id = ds.user_id" .
            ...
            $sql = "INSERT INTO " . $this->ecs->table('order_goods') . "( " .
                    ...
                    "goods_attr, is_real, extension_code, parent_id, is_gift, model_attr, goods_attr_id, ru_id, shopping_fee, warehouse_id, area_id, is_distribution, drp_money) " .
                    ...
                    "ca.is_real, ca.extension_code, ca.parent_id, ca.is_gift, ca.model_attr, ca.goods_attr_id, ca.ru_id, ca.shopping_fee, ca.warehouse_id, ca.area_id," .
                    ...
                    " WHERE ca." . $this->sess_id . " AND ca.rec_type = '$flow_type'" . $goodsIn;
            $this->db->query($sql);
            //微分销end
        } else {
            $goodsIn = '';
        ...
        if (is_dir(APP_WECHAT_PATH)) {
            $pushData = array(
                'orderID' => array('value' => $order['order_sn'], 'color' => '#173177'), //订单号
                'orderMoneySum' => array('value' => $order['order_amount'], 'color' => '#173177'), //订单金额
						...
						if($res!='国内仓库')
						{
							...
							foreach($val['goods_list'] as $key1 => $val1)
							{
								...
								if($free_rate != 1)
								{
									...
									$rate_price = round($val1['subtotal'] *  ($rate / 100),2);
...
//
    ...
    public function actionAddAddress() {
        ...
        if (IS_POST) {
            ...
            } else if (isset($_SESSION['flow_consignee']) && !empty($consignee['address_id'])) {
