<?php
namespace App\Custom\User\Controllers;

use App\Http\User\Controllers\Order as Foundation;
...
class Order extends Foundation
{
    ...
    public function actionIndex()
    {
        ...
        if (IS_POST) {
			...
			{
				$sql="SELECT order_amount FROM ".$GLOBALS['ecs']->table('order_info')." WHERE order_id = '{$val['order_id']}'";
				$order_list['list'][$key]['order_amount'] = price_format($GLOBALS['db']->getOne($sql), false);
			}
...
//        $this->assign('page_title', '订单列表');
    ...
    public function actionSurplusPay()
    {
        ...
        if (is_dir(APP_WECHAT_PATH)) {
            ...
            $pushData = array(
                'keyword1' => array('value' => $order['pay_time'], 'color' => '#173177'), //变动时间
                'keyword2' => array('value' => '消费扣减', 'color' => '#173177'), //变动类型
                'keyword3' => array('value' => $surplus, 'color' => '#173177'), // 变动金额
                'keyword4' => array('value' => $users['user_money'], 'color' => '#173177'), // 当前余额
