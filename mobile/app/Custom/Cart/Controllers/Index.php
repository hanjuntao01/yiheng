...
use App\Http\Cart\Controllers\Index as Foundation;
					...
					if($res!='国内仓库')
					{
						...
						foreach($val['goods_list'] as $key1 => $val1)
						{
							...
							if($free_rate != 1)
							{
								...
								$rate_price = round(str_replace('¥','',$val1['goods_price']) *  ($rate / 100),2);
								//echo round(doubleval($cart_goods['total']['goods_amount'])+doubleval($rate_price),2);exit;
								//print_r($cart_goods['total']);exit;
								...
								//echo $cart_goods['total']['goods_price'];exit;
