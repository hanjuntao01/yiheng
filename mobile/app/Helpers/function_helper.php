...
{
    $sql = "SELECT g.goods_id, g.goods_name, g.goods_thumb, og.goods_number, og.goods_price, og.goods_price, og.extension_code, og.goods_name AS extension_name, oi.order_sn, ga.act_id FROM " . $GLOBALS['ecs']->table('order_goods') . " as og " .
    ...
    foreach ($res as $key => $row) {
        ...
        if ($row['extension_code'] == 'package_buy') {
            ...
            $arr[$key]['goods_thumb'] = $activity['activity_thumb'];
        }
    }
...
function get_goods_transport_tpl($goodsInfo = array(), $region = array(), $shippingInfo = array(), $goods_number = 1)
{
    ...
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
        ...
        $GLOBALS['ecs']->table('area_region') . ' AS r ' .
        'WHERE r.region_id ' . db_create_in($region) .
        " AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.shipping_id = '" . $shippingInfo['shipping_id'] . "' AND s.enabled = 1 AND a.ru_id = '" . $goodsInfo['user_id'] . "'";
    $val = $GLOBALS['db']->getRow($sql);
