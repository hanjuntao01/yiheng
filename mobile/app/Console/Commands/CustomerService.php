            ...
            switch ($data['type']) {
                case 'login':
                    ...
                    if ($data['user_type'] == 'service') {
                        ...
                        $connection->targetService = array(
                ...
                case 'sendmsg':
                     ...
                     * 发送消息
                     */
                    $msg = array('from_id' => $connection->uid, 'name' => $connection->uname, 'time' => date('H:i:s'), 'message' => $data['msg'], 'avatar' => $data['avatar'], 'goods_id' => $data['goods_id'], 'store_id' => $data['store_id'], 'message_type' => 'come_msg');
                    ...
                    if ($connection->userType == 'customer') {
                        ...
                        if ( $ws_worker->customerContainer[$data['to_id']]->targetService['store_id'] == $data['store_id']
                            ...
                            // 当前没在聊天
                            // 设置客户当前聊天对象
                            $ws_worker->customerContainer[$data['to_id']]->targetService = array(
                                'store_id' => $data['store_id'],
                                'sid' => $connection->uid
                            );
                            $msg['status'] = 0;
                        ...
                        elseif ( $ws_worker->customerContainer[$data['to_id']]->targetService['store_id'] == $data['store_id']
                            ...
                            // 客户在跟别人聊天
                            if ( $ws_worker->customerContainer[$data['to_id']] -> origin == 'phone' ) {
                                // 手机登录 存为离线消息
                                $msg['to_id'] = $data['to_id'];
                                $msg['status'] = 1;
                                $event->savemsg($msg);
                ...
                case 'info':
                    ...
                    if (isset($ws_worker->customerContainer[$data['msg']])) {
                        ...
                        // 设置客户当前聊天对象
                        $ws_worker->customerContainer[$data['msg']]->targetService = array(
                            'store_id' => $data['store_id'],
                            'sid' => $data['from_id']
                        );
                    }
                ...
                case 'change_service':
                    ...
                    if ($connection->userType == 'service') {
                        if (
                            ...
                            $ws_worker->customerContainer[$data['cus_id']]->targetService = array(
                                'store_id' => $data['store_id'],
                                'sid' => $data['to_id']
                            );
                            //通知客户
                            $msg = array('sid' => $data['to_id'], 'fid' => $data['from_id'], 'store_id' => $data['store_id'],
