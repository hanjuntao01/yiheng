...
function goods_count_by_brand($brand_id, $mbid = 0, $cate = 0, $act = '', $ship = 0, $price_min = 0, $price_max = 0, $warehouse_id = 0, $area_id = 0, $self = 0, $type = '')
{
    ...
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        ...
        "WHERE " . $type_sql . "  g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND $brand_where $cate_where $tag_where ";
...
function brand_get_goods($brand_id, $mbid = 0, $cate, $size, $page, $sort, $order, $warehouse_id = 0, $area_id = 0, $act = '', $ship = '', $price_min, $price_max, $type = '')
{
    ...
    $brand_where .= "((g.brand_id = '$brand_id' AND g.user_id = 0) $merchants_brands)";
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.user_id, g.goods_name, g.market_price, g.is_new,g.is_promote, g.is_best,g.is_hot,g.shop_price AS org_price,g.sales_volume, ' .
        $shop_price .
        "IFNULL(IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * '$_SESSION[discount]'), g.shop_price * '$_SESSION[discount]')  AS shop_price, " .
        "IFNULL(IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)), g.promote_price) AS promote_price, " .
...
function brand_related_cat($brand)
{
    $arr[] = array(
        ...
        'url' => url('brand/index/detail', array('id' => $brand))
    );
