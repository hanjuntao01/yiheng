...
class Admin extends Backend
{
    ...
    public function actionTeaminfo()
    {
        ...
        $sql_count = "select count(*) as count from {pre}team_log as tl LEFT JOIN {pre}order_info as oi ON tl.team_id = oi.team_id LEFT JOIN {pre}team_goods as tg ON tl.t_id = tg.id LEFT JOIN {pre}goods as g ON tl.goods_id = g.goods_id  where is_show = 1 and oi.team_parent_id > 0 $where order by tl.start_time desc ";
        $total = $this->model->getOne($sql_count);
