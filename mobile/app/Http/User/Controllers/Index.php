...
class Index extends Frontend
{
    ...
    public function actionHistory()
    {
        ...
        $sql = 'SELECT count(goods_id) FROM {pre}goods' .
        ...
        if (IS_AJAX) {
            ...
            die(json_encode(array('history' => $history['goods_list'], 'totalPage' => $page_size, 'count' => $count)));
        }
    ...
    public function actionAddAddress()
    {
        if (IS_POST) {
            ...
            // 验证所在地区
            if (empty($consignee['province']) || empty($consignee['city']) || empty($consignee['district'])) {
                exit(json_encode(array('status' => 'n', 'info' => L('select_Local_area'))));
            }
            // 收货地址数量限制
