...
class Admin extends Backend
{
        ...
        if (IS_POST) {
            ...
            if (C('shop.open_oss') == 0 && !empty($pic_path)) {
                ...
                if ($img_info[0] != 640 || $img_info[1] != 1136) {
                    exit(json_encode(array('error' => 1, 'msg' => '图片不符合规定,请上传宽高为640*1136图片')));
                }
            }
            ...
            if ($_FILES['pic']['name']) {
                ...
                if (C('shop.open_oss') == 1) {
                    $imglist = array('0' => basename($result['url']));
    ...
    public function actionSynchroImages()
    {
        if (IS_AJAX) {
            ...
            // 获得文件列表
            if ($file_list) {
                $file_list = array_values(array_diff($file_list, array('..','.')));
            ...
            if ($type == 0) {
                if (!empty($imglist)) {
                    ...
                    if ($res == true) {
                        $result = array('error' => 0, 'msg' => '同步上传OSS成功');
                    } else {
                        $result = array('error' => 1, 'msg' => '同步上传OSS失败');
                    }
                    exit(json_encode($result));
                } else {
                    $result = array('error' => 1, 'msg' => '没有图片可同步上传');
                    exit(json_encode($result));
                }
            }
            // 同步下载
            if ($type == 1){
                if (!empty($imglist)) {
                    ...
                    if ($res == true) {
                        $result = array('error' => 0, 'msg' => '同步下载成功');
                    } else {
                        $result = array('error' => 1, 'msg' => '同步下载失败');
                    }
                    exit(json_encode($result));
                } else {
                    $result = array('error' => 1, 'msg' => '没有图片可同步下载');
                    exit(json_encode($result));
                }
            }
        }
    ...
    public function actionDrpEditCredit()
    {
        ...
        if (IS_POST) {
            $data = I('data', '', 'trim');
            if (!empty($data)) {
                dao('drp_shop')->data($data)->where(array('id' => $data['id']))->save();
                $result = array('error' => 0, 'msg' => '编辑成功');
                exit(json_encode($result));
            }
        }
        $id = I('id', 0, 'intval');
        $shop = dao('drp_shop')->field('user_id')->where(array('id' => $id))->find();
    ...
    public function actionDrpCount()
    {
        ...
        if (IS_AJAX) {
            ...
            $tooltip = array(
                'trigger' => 'axis',
                'axisPointer' => array('lineStyle' => array('color' => '#6cbd40'))
            );
            $xAxis = array(
                'type' => 'category',
                'boundaryGap' => false,
