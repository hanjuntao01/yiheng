...
class User extends Frontend
{
    ...
    public function __construct()
    {
        ...
        //$this->update_drp_parent();//更新分销商 drp_parent_id 父级id
        // 用户信息
    ...
    public function actionIndex()
    {
        ...
        $this->assign('url', url('drp/shop/index', array('id' => $this->drp['drp_id'], 'd' => $_SESSION['user_id'])));//我的微店
        ...
        if ($generate) {
            ...
            if ($qrcodeInfo) {
                ...
                if ($is_show_avatar == false) {
                    // 生成背景图加二维码
                    $img->open($outImg)->text($text_description, dirname(ROOT_PATH) . '/data/attached/fonts/msyh.ttf', $font_size, $text_color, array($av_left + 100 + 20, $av_top + 10))->save($outImg);
                } else {
                    // 生成背景图加二维码+微信头像
                    $img->open($outImg)->water($avatar, array($av_left, $av_top), 100)->text($text_description, dirname(ROOT_PATH) . '/data/attached/fonts/msyh.ttf', $font_size, $text_color, array($av_left + 100 + 20, $av_top + 10))->save($outImg);
                }
            }
            // 同步OSS数据
