...
function category_get_goods($children, $intro = '', $brand, $user_id, $sort = '', $warehouse_id = 0, $area_id = 0, $limit = 10)
{
    ...
    if ($intro) {
        switch ($intro) {
            case 'best':
                $where .= ($user_id > 0 && $goodsnum > 0) ? " AND  g.store_best = '1' " : (($user_id > 0) ? " AND 1 " : " AND is_best = '1' ");
                break;
            case 'new':
    ...
    foreach ($res as $row) {
        ...
        $arr[$row['goods_id']]['price'] = ($promote_price > 0) ? price_format(min($promote_price, $row['shop_price'])) : '';
        $arr[$row['goods_id']]['img'] = get_wechat_image_path($row['goods_thumb']);
        $arr[$row['goods_id']]['goods_img'] = get_wechat_image_path($row['goods_img']);
