...
class View extends Frontend
{
    ...
    public function actionStoreIn()
    {
        if (IS_POST) {
            ...
            foreach ($store as $key => $value) {
                ...
                $sql = "SELECT count(*) FROM " . $GLOBALS['ecs']->table('goods') .
                    " where user_id = '" .$value['user_id']. "' AND is_on_sale = 1 AND is_delete = 0 AND is_alone_sale = 1 AND review_status > 2 AND is_promote = 1 ";
    ...
    public function actionStoreDown()
    {
        if (IS_POST) {
            $ru_id = input('ruid');
            $time = gmtime();
            $sql = "SELECT ms.shop_id, ms.user_id, ms.rz_shopName,ss.kf_qq, ss.kf_ww, ss.meiqia " .
                " FROM " . $GLOBALS['ecs']->table('merchants_shop_information') . " AS ms " .
                " LEFT JOIN " . $GLOBALS['ecs']->table('seller_shopinfo') . " AS ss " .
                " ON ms.user_id = ss.ru_id  " .
                " WHERE ms.user_id = $ru_id  ";
