...
class Index extends Frontend
{
    ...
    public function actionIndex()
    {
        ...
        if ($goods_id) {
            $goods = dao('goods')->field('goods_id, goods_name, shop_price, goods_thumb, user_id')
                ...
                ->find();
            $goods['goods_thumb'] = $rootUrl . '/' . $goods['goods_thumb'];
     ...
     * 根据用户信息查询订单列表
     */
    private function orderListByUid ($uid) {

        if (empty($uid)) {
            return [];
        }
        $sql = 'SELECT oi.order_sn, oi.order_amount+oi.money_paid as order_amount, oi.add_time as order_time, g.goods_id, g.goods_name, g.goods_thumb FROM {pre}order_info oi';
        $sql .= " LEFT JOIN {pre}order_goods og ON oi.order_id = og.order_id";
        $sql .= " LEFT JOIN {pre}goods g ON g.goods_id = og.goods_id";
        $sql .= ' WHERE oi.user_id = ' . $uid . ' ORDER BY oi.order_id DESC';
