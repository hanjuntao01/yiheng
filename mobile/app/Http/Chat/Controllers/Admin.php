...
class Admin extends Foundation
{
    protected $config = [];
    ...
    {
        $uid = I('uid', 0, 'intval');
        $cidd = I('cidd', 0, 'intval');

        $dialog = Kefu::getRecentDialog($uid, $cidd);
        ...
        $dialogInfo = array(
            ...
            'name' => $user['user_name'],
            'services_id' => $uid,
            'goods' => Kefu::getGoods($dialog['goods_id']),
            'store_id' => $dialog['store_id'],
            'start_time' => $dialog['start_time'],
            'origin' => ($dialog['origin'] == 1) ? "PC" : "mobile",
        );
    ...
    public function actionCreatedialog()
    {
        ...
        $cidx = I('cidx', 0, 'intval');  //客户

        $dialog = Kefu::getRecentDialog($fid, $cidx);

        Kefu::addDialog(array(
            'customer_id' => $dialog['customer_id'],
            'services_id' => $uid,
