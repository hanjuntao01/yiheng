...
use App\Http\Base\Controllers\BackendSeller;
use App\Extensions\Wechat;
use App\Extensions\Form;
use App\Extensions\Http;

class Seller extends BackendSeller
{
    // 微信对象
    protected $weObj = '';
    // 微信公众号ID
    ...
    public function actionExtendEdit()
    {
        ...
        $plugin = '\\App\\Http\\Wechat\\Plugins\\' . ucfirst($this->plugin_name) . '\\' . ucfirst($this->plugin_name);
    ...
    public function actionMarketList()
    {
        ...
        $plugin = '\\App\\Http\\Wechat\\Market\\' . ucfirst($this->market_type) . '\\' . 'Admin';
        if (class_exists($plugin)) {
    ...
    public function actionMarketAction()
    {
        ...
        if (file_exists($file)) {
            include_once($file);
            $market = '\\App\\Http\\Wechat\\Market\\' . ucfirst($this->market_type) . '\\' . 'Admin';
