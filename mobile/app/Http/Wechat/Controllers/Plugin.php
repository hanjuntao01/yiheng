...
abstract class Plugin extends Foundation
{
    ...
    public function plugin_display($tpl = '', $config = array())
    {
        ...
        if ($_SESSION['seller_id'] > 0) {
            $tpl = 'app/Http/' . MODULE_NAME . '/' . $plugin_path . '/' . ucfirst($this->plugin_name) . '/Views_seller/' . $tpl . C('TMPL_TEMPLATE_SUFFIX');
        } else {
            $tpl = 'app/Http/' . MODULE_NAME . '/' . $plugin_path . '/' . ucfirst($this->plugin_name) . '/Views/' . $tpl . C('TMPL_TEMPLATE_SUFFIX');
        }
