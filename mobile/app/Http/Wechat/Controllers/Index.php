...
class Index extends Frontend
{
    ...
    public function get_function($fromusername, $keywords)
    {
        ...
        if (file_exists($file)) {
            require_once($file);
            $new_command = '\\App\\Http\\Wechat\\Plugins\\' . ucfirst($info['command']) . '\\' . ucfirst($info['command']);
    ...
    public function send_message($fromusername, $keywords, $weObj, $return = 0)
    {
        ...
        if (file_exists($file)) {
            require_once($file);
            $new_command = '\\App\\Http\\Wechat\\Plugins\\' . ucfirst($rs['command']) . '\\' . ucfirst($rs['command']);
            $cfg = array('ru_id' => $this->ru_id);
            $wechat = new $new_command($cfg);
    ...
    public function actionPluginShow()
    {
        ...
        if (file_exists($file)) {
            include_once($file);
            $new_plugin = '\\App\\Http\\Wechat\\Plugins\\' . ucfirst($plugin_name) . '\\' . ucfirst($plugin_name);
    ...
    public function actionMarketShow()
    {
        ...
        if (file_exists($file) && !empty($function)) {
            include_once($file);
            $market = '\\App\\Http\\Wechat\\Market\\' . ucfirst($market_type) . '\\' . ucfirst($market_type);
