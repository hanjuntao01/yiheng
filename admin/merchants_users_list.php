...
if ($_REQUEST['act'] == 'add_shop' || $_REQUEST['act'] == 'edit_shop' || $_REQUEST['act'] == 'copy_shop')
{
    ...
    $sql="SELECT source FROM ".$GLOBALS['ecs']->table('merchants_steps_fields')." WHERE user_id='$user_id'";
    $source=$GLOBALS['db']->getOne($sql);
    $smarty->assign('source',    $source);
    $sql="SELECT country FROM ".$GLOBALS['ecs']->table('merchants_steps_fields')." WHERE user_id='$user_id'";
    $c_id=$GLOBALS['db']->getOne($sql);
    $sql="SELECT id FROM ".$GLOBALS['ecs']->table('country')." WHERE id='$c_id'";
    $country_id=$GLOBALS['db']->getOne($sql);
    $smarty->assign('country_id',    $country_id);
    $sql="SELECT * FROM ".$GLOBALS['ecs']->table('country');
    $list_country=$GLOBALS['db']->getAll($sql);
    $smarty->assign('list_country',$list_country);  // ecmoban hky
	 /*删除未绑定品牌 by kong*/
...
elseif ($_REQUEST['act'] == 'insert_shop' || $_REQUEST['act'] == 'update_shop')
{
    ...
    if($_REQUEST['act'] == 'update_shop'){ //更新数据
        ...
        $parent['country'] = isset($_REQUEST['m_country']) ? intval($_REQUEST['m_country']) : 0;
        ...
        if($merchants_audit != 1){
            ...
            $db->query("UPDATE " . $ecs->table('goods') . " AS g SET g.is_on_sale = 1 WHERE g.user_id = '$user_id' $where"); //审核未通过下架商家所有商品
        }
