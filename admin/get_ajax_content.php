...
elseif ($_REQUEST['act'] == 'drop_seckill_goods') {
    ...
    foreach ($arr AS $val) {
        $opt[] = array(
			'value' => $val['id'],
            'text' => '【价格：' . $val['sec_price'] . '|数量：' .$val['sec_num'].'|限购：'.$val['sec_limit'].'】' .$val['goods_name'],
            'data' => '');
    }
...
//编辑导航名称
elseif($_REQUEST['act'] == 'edit_navname_home'){
    $json = new JSON;
    $result = array('goods_desc'=>0);
...
//编辑导航链接
elseif($_REQUEST['act'] == 'edit_navurl_home'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'pic_id' => '','content' => '');
...
/*编辑导航排序*/
elseif($_REQUEST['act'] == 'edit_navvieworder_home'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'pic_id' => '','content' => '');
...
/*切换导航显示*/
elseif($_REQUEST['act'] == 'edit_ifshow_home'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'pic_id' => '','content' => '');
    $id = isset($_REQUEST['id'])  ? intval($_REQUEST['id']) : 0;
...
elseif($_REQUEST['act'] == 'remove_nav_home'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'pic_id' => '','content' => '');
    $id = isset($_REQUEST['id'])  ? intval($_REQUEST['id']) : 0;
...
/*添加导航*/
elseif($_REQUEST['act'] == 'add_nav_home'){
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => '', 'pic_id' => '','content' => '');
    $exc   = new exchange($ecs->table("nav"), $db, 'id', 'name');
...
elseif($_REQUEST['act'] == 'nav_mode'){
    ...
    $result['navColor'] = $spec_attr['navColor'];
        ...
        $sql = "SELECT name, url,opennew " .
                " FROM " . $GLOBALS['ecs']->table('nav') . $where .
                " ORDER by vieworder LIMIT 9";
