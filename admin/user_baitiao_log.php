...
function getbaitiao_list($user_id = 0){
    ...
    if ($result === false)
    {
        ...
        /* 分页大小 */
        $filter = page_and_size($filter);
        $sql = "SELECT b.*,o.order_sn,o.order_amount FROM ".$GLOBALS['ecs']->table('baitiao_log')." AS b LEFT JOIN ".$GLOBALS['ecs']->table('order_info')."  AS o ON b.order_id=o.order_id  $ex_where ORDER BY b.log_id DESC".
                " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
