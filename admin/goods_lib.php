...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($_POST['goods_sn'])
    {
        $sql = "SELECT COUNT(*) FROM " . $ecs->table('goods_lib') .
                " WHERE goods_sn = '$_POST[goods_sn]' AND goods_id <> '$_POST[goods_id]'";
        if ($db->getOne($sql) > 0)
        {
	...
	*/
    ...
    {
        $max_id     = $is_insert ? $db->getOne("SELECT MAX(goods_id) + 1 FROM ".$ecs->table('goods_lib')) : $_REQUEST['goods_id'];
    ...
    if ($result['list']) {
		...
		if(!$GLOBALS['db']->getOne($sql)){
			...
			$sql = "INSERT INTO " . $ecs->table('goods_lib') .
					...
					" '$goods_info[cost_price]', '$goods_info[shop_price]', '$goods_info[keywords]', '$goods_info[goods_brief]', '$goods_info[goods_desc]', '$goods_info[desc_mobile]', '$goods_thumb', '$goods_img', '$original_img', ".
