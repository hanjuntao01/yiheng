...
function get_admin_userlist($ru_id)
{
    ...
    if ($result === false) {
        ...
        $sql = 'SELECT user_id, user_name, au.ru_id, email, add_time, last_login, agency_id ' .
                'FROM ' . $GLOBALS['ecs']->table('admin_user') . ' AS au '.
                'WHERE 1 AND parent_id = 0 AND ru_id = 0 '. $where .' ORDER BY user_id DESC ' .
