...
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    /* 取得发货单商品 */
    $goods_sql = "SELECT dg.*, g.goods_thumb  FROM " . $ecs->table('delivery_goods') . 
...
elseif ($_REQUEST['act'] == 'back_info')
{
    ...
    /* 取得发货单商品 */
    $goods_sql = "SELECT b.*, g.goods_thumb FROM " . $ecs->table('back_goods') ." AS b LEFT JOIN ".$ecs->table('goods')." AS g ON g.goods_id = b.goods_id ".
...
elseif ($_REQUEST['act'] == 'edit_refound_amount')
{
    ...
    if($type == 1){
        ...
        /* 退款 */
        if ($ret_d > 0) {
            $refound_fee = order_refound_fee($order_id, $ret_d); //已退金额
...
function update_order_amount($order_id)
{
    ...
    $sql="SELECT SUM(rate_price) FROM ".$GLOBALS['ecs']->table('order_goods')." WHERE order_id = '$order_id'";
    $rate_price=$GLOBALS['db']->getOne($sql);
    //更新订单总金额
    $sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') .
            " SET order_amount = " . order_due_field() .
            "+$rate_price WHERE order_id = '$order_id' LIMIT 1";
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT ifnull(bai.is_stages,0) is_stages, o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.is_delete," .
                ...
                "o.shipping_time, o.auto_delivery_time, o.pay_status, o.consignee, o.address, o.email, o.tel, o.mobile, o.extension_code as o_extension_code, o.extension_id, o.is_zc_order, o.pay_id, " .
                "o.pay_name, o.referer, o.froms, o.user_id, o.chargeoff_status, " .
                "(" . order_amount_field('o.') . ") AS total_fee, (o.goods_amount - o.discount + o.tax + o.shipping_fee + o.insure_fee + o.pay_fee + o.pack_fee + o.card_fee) AS total_fee_order" .
...
function download_orderlist($result) {
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        $ru_name = !empty($data_cnt) ? i($result[$i]['shop_name']) . ',' : ''; //商家名称
