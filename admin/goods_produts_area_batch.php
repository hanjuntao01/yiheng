    ...
    if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
        ...
        if (!empty($result['list']['goods_attr'])) {
            ...
            $sql = "SELECT product_id FROM " . $GLOBALS['ecs']->table('products_area') . " WHERE goods_id = '" . $result['list']['goods_id'] . "'" .
                    " AND area_id = '" . $result['list']['area_id'] . "'" . $where;
            $res = $GLOBALS['db']->getOne($sql);

            if ($GLOBALS['db']->getOne($sql)) {
                $db->autoExecute($ecs->table('products_area'), $other, 'UPDATE', "goods_id = '" . $result['list']['goods_id'] . "'" .
                        " AND area_id = '" . $result['list']['area_id'] . "'" . $where);
...
elseif ($_REQUEST['act'] == 'download') {
    ...
    $goods_id = isset($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : 0;
    $model = isset($_REQUEST['model']) ? intval($_REQUEST['model']) : 0;
    $area_id = isset($_REQUEST['area_id']) ? intval($_REQUEST['area_id']) : 0; //当前地区ID bylu;
