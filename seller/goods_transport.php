...
$smarty->assign('menu_select', array('action' => '02_cat_and_goods', 'current' => '01_goods_list')); //商家后台菜单栏
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('goods_transport_list.dwt'), '',
        array('filter' => $transport_list['filter'], 'page_count' => $transport_list['page_count']));
}
...
elseif ($_REQUEST['act'] == 'get_shipping_tem') 
{
    ...
    $sql = "SELECT a.shipping_name, a.shipping_code, a.support_cod, b.* " .
            ...
            " WHERE b.tid='" . $tid . "' and b.shipping_id='" . $shipping_id . "' and b.user_id='" . $adminru['ru_id'] . "'";
    $row = $db->getRow($sql);
    if (!empty($row)) {
        $set_modules = 1;
