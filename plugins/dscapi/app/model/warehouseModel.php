...
abstract class warehouseModel extends common {
    ...
    public function get_insert($table, $select, $format){
        ...
        $common_data = array(
            ...
            'msg' => empty($id) ? $warehouseLang['msg_failure']['failure'] : $warehouseLang['msg_success']['success'],
            'error' => empty($id) ? $warehouseLang['msg_failure']['error'] : $warehouseLang['msg_success']['error'],
            'format' => $format
        );
    ...
    public function get_update($table, $select, $where, $format){
        ...
        if (strlen($where) != 1) {
            ...
            if (!$info) {
                $common_data = array(
                    'result' => 'failure',
                    'msg' => $warehouseLang['null_failure']['failure'],
                    'error' => $warehouseLang['null_failure']['error'],
                    'format' => $format
                );
            } else {
                ...
                $common_data = array(
                    ...
                    'msg' => empty($select) ? $warehouseLang['msg_failure']['failure'] : $warehouseLang['msg_success']['success'],
                    'error' => empty($select) ? $warehouseLang['msg_failure']['error'] : $warehouseLang['msg_success']['error'],
                    'format' => $format
                );
            }
        } else {
